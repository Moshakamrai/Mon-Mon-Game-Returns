...
public class DynamicTextManager : MonoBehaviour
{
    ...
    public static GameObject canvasPrefab2;
    ...
    private void Awake()
    {
        ...
        canvasPrefab = _canvasPrefab[0];
        canvasPrefab2 = _canvasPrefab[1]; 
    }
    ...
    public static void CreateText2DNew(Vector2 position, string text, DynamicTextData data)
    {

        GameObject newText = Instantiate(canvasPrefab2, position, Quaternion.identity);
        newText.transform.GetComponent<DynamicText2D>().Initialise(text, data);
    }

    public static void CreateTextNew(Vector3 position, string text, DynamicTextData data)
    {
        GameObject newText = Instantiate(canvasPrefab2, position, Quaternion.identity);
        newText.transform.GetComponent<DynamicText>().Initialise(text, data);
    }
